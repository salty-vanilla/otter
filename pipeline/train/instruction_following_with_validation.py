...
def train_one_epoch(
    ...
    num_batches_per_epoch = len(train_loaders[0])
    total_training_steps = num_batches_per_epoch * args.num_epochs

    # special design for Idefics Model's prompt strategy
    fake_token_image_exists = (
        True
        if "<fake_token_around_image>"
        in tokenizer.special_tokens_map["additional_special_tokens"]
        else False
    )
    fake_token_image_token_id = tokenizer(
        "<fake_token_around_image>", add_special_tokens=False
    )["input_ids"][-1]
    ...
    endofchunk_token_id = tokenizer(endofchunk_text, add_special_tokens=False)[
        "input_ids"
    ][-1]
    answer_token_id = tokenizer("<answer>", add_special_tokens=False)["input_ids"][-1]
    ens_token_id = tokenizer(tokenizer.eos_token, add_special_tokens=False)[
        "input_ids"
    ][-1]
    ...
    for num_steps, (batch_mimicits) in tqdm(
        ...
        for batch_mimicit in batch_mimicits:
            images = batch_mimicit["net_input"]["patch_images"].to(
            ...
            with accelerator.autocast():
                ...
                if IdeficsForVisionText2Text is not None and isinstance(
                    unwrapped_model, IdeficsForVisionText2Text
                ):
        ...
        if accelerator.sync_gradients:
            ...
            if (
                ...
                print(
                    f"Saving checkpoint to {args.external_save_dir}/checkpoint_steps_{global_step}.pt"
                )
                accelerator.save(
                    checkpoint_dict,
        ...
        if ((num_steps + 1) % args.logging_steps == 0) and args.rank == 0:
            print(
                f"Step {num_steps+1}/{num_batches_per_epoch} of epoch {epoch+1}/{args.num_epochs} complete."
                f"Loss MIMIC-IT: {mean_loss.item():.3f}"
            )

        accelerator.wait_for_everyone()
        if ((num_steps + 1) % args.validation_steps == 0) and args.rank == 0:
            execute_validation(
                args=args,
                model=model,
                epoch=epoch,
                tokenizer=tokenizer,
                mimicit_loaders=val_loaders,
                accelerator=accelerator,
                device_id=device_id,
                wandb=wandb,
                current_step=global_step // args.gradient_accumulation_steps,
            )
            accelerator.wait_for_everyone()
            model.train()
...
def execute_validation(
    ...
    total_eval_steps = num_batches_per_epoch * 1
    ...
    accelerator.print(f"Val Loss MIMIC-IT: {mean_loss.item():.3f}")
...
def main():
    ...
    if args.pretrained_model_name_or_path is not None:
        ...
        if "otter" in args.model_name.lower():
            ...
            processor = AutoProcessor.from_pretrained(
                args.pretrained_model_name_or_path, legacy=False
            )
            past_special_tokens = processor.tokenizer.special_tokens_map[
    ...
    train_loaders, val_loaders = get_data(args, image_processor, tokenizer, "mimicview")
    ...
    total_training_steps = len(train_loaders[0]) * args.num_epochs
    ...
    if (
        accelerator.distributed_type == "DEEPSPEED"
        or accelerator.distributed_type == "MULTI_GPU"
    ):
    ...
    for epoch in range(resume_from_epoch, args.num_epochs):
        for cur_data_loader in train_loaders:
        ...
        # # Validation
        # for cur_data_loader in val_loaders:
        #     cur_data_loader.dataset.set_epoch(epoch)
        # start = time.time()
        # val_one_epoch(
        #     args=args,
        ...
        if args.save_ckpt_each_epoch:
            ...
            if args.rank == 0:
                print(
                    f"Saving checkpoint to {args.external_save_dir}/checkpoint_{epoch}.pt"
                )
                accelerator.save(
                    checkpoint_dict, f"{args.external_save_dir}/checkpoint_{epoch}.pt"
                )
                # save the config
